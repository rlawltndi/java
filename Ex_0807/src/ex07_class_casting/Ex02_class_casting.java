...
public class Ex02_class_casting {
	public static void main(String[] args) {
		Car c1= new SchoolBus();//1차 상속관계가 아니더라도 자동 타입변환이 가능
		Bus b1 = new Bus();
		...
		//타입을 부모타읍으로 변환한 객체는 더이상 자신의 클래스에 차가한 멤버를
		//사용 할 수 없다.
		//부모클래스에 선언된 멤버만 사용가능.
		//단. 부모클래스의 메서드를 오버라이딩 한 경우 메서드의 경우에는
		//자식 객체의 것을 호출할 수 있다.
		
		//어떻게 타입변환을 해도 오버라이딩된 메서드가 호출이 될까?
		//메서드가 실행 시점에 성격이 결정되는 동적바인딩을 하기 때문이다.
		//프로그램의 컴파일 시점에 실행되는 메서드가 부모클래시의 것인지
		//지삭클래스의 것인지 알기 어렵다.
		...
		//-이 때 , 자식 클래스에서 부모 클래스의 동일한 시그니쳐(메서드명 ,매개변수)
		//를 가진 메서드를 재정의한다.
